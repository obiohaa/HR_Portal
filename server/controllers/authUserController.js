...
const login = async (req, res) => {
  ...
  const refreshTokenExpiry = new Date(Date.now() + longDay);
  const tokenUser = createTokenUser(user, refreshTokenExpiry);

  //Create refresh token and hash it before saving to database
  let refreshToken = " ";
  ...
  //This updates the user refreshToken and attach it to cookies
  if (existingToken) {
    refreshToken = crypto.randomBytes(40).toString("hex");
    const userTokenUpdated = await Token.findOneAndUpdate(
      { user: user._id },
      {
        userAgent: req.headers["user-agent"], //another was to get attributes from request like req.get('origin')
        ip: req.ip,
        refreshToken,
        lastLogin: Date.now(),
        expiredIn: refreshTokenExpiry,
      },
      {
        new: true,
        runValidators: true,
